...
interface AnalysisResult {
  ...
  insights: any[];
  stats?: { files: number; functions: number; classes: number; dependencies: number; circularDeps: number }; entityCounts?: { modules: number; functions: number; classes: number; dependencies: number; circularDeps: number }; totalFilesAnalyzed?: number;
}
...
server.tool(
  ...
  async ({ project, type, limit }) => {
    ...
    const result = {
      ...
      truncated,
      stats: (loaded.analysis.stats || loaded.analysis.entityCounts),
...
server.tool(
  ...
  async ({ project, businessRule, changeDescription }) => {
    ...
    const systemMapContent = [
      ...
      "",
      `## Overview`,
      `- **Modules**: ${(loaded.analysis.stats?.files || loaded.analysis.entityCounts?.modules || loaded.analysis.totalFilesAnalyzed || 0)}`,
      `- **Functions**: ${((loaded.analysis.stats || loaded.analysis.entityCounts)?.functions || loaded.analysis.entityCounts?.functions || 0)}`,
      `- **Classes**: ${((loaded.analysis.stats || loaded.analysis.entityCounts)?.classes || loaded.analysis.entityCounts?.classes || 0)}`,
      `- **Dependencies**: ${((loaded.analysis.stats || loaded.analysis.entityCounts)?.dependencies || loaded.analysis.entityCounts?.dependencies || 0)}`,
      `- **Circular Deps**: ${((loaded.analysis.stats || loaded.analysis.entityCounts)?.circularDeps || loaded.analysis.entityCounts?.circularDeps || 0)}`,
      "",
