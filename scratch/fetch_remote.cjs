      ...
      if (p.name.includes('auto-edit-video-reup')) {
        ...
        if (analysisRes.data) {
          ...
          if (base.graph && base.graph.nodes) {
            const moduleNodes = base.graph.nodes.filter(n => n.type === 'module');
            console.log(`Modules (${moduleNodes.length}) sample:`, JSON.stringify(moduleNodes.slice(0, 3), null, 2));
            const modules = moduleNodes.map(n => n.name || n.label || n.id);
            console.log(`Modules clean names:`, modules);
          }
        } else {
