...
- Files skipped: ${result.totalFilesSkipped}`;
  ...
  server.tool(
    ...
    async ({ project, businessRule, changeDescription, enableEnterpriseSync }: { project?: string; businessRule?: string; changeDescription?: string; enableEnterpriseSync?: boolean }) => {
      ...
      await fs.promises.writeFile(path.join(memoryDir, "modules.json"), JSON.stringify(modulesJson, null, 2));
      ...
      await fs.promises.writeFile(path.join(memoryDir, "feature-flows.json"), JSON.stringify(featureFlows, null, 2));

      // business-rules.json
      const businessRulesPath = path.join(memoryDir, "business-rules.json");
      let businessRules: Array<{ rule: string; addedAt: string }> = [];
      if (await fileExists(businessRulesPath)) {
        try {
          const parsedBR = JSON.parse(await fs.promises.readFile(businessRulesPath, "utf-8"));
      ...
      await fs.promises.writeFile(businessRulesPath, JSON.stringify(businessRules, null, 2));
      ...
      if (await fileExists(changeLogPath)) {
        try {
          const parsedCL = JSON.parse(await fs.promises.readFile(changeLogPath, "utf-8"));
