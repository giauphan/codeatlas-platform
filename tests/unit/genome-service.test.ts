...
safeMockModule(
  path.join(srcDir, "services/embeddingService.js"),
  embeddingMock,
);
...
describe("GenomeService", () => {
  ...
  describe("upsertGene()", () => {
    ...
    test("updates existing gene (v1→v2) and records mutation", async () => {
      ...
      mockConnection.execute.mock.mockImplementation(
        async (sql: string, binds: any) => {
          if (sql.includes("SELECT id, version FROM codeatlas_genome")) {
      ...
      const mutationCalls = mockConnection.execute.mock.calls.filter(
        (c: any) =>
          ...
          c.arguments[0].includes("INSERT INTO gene_mutations"),
      );
  ...
  describe("mergeGenes()", () => {
    test("combines 2 genes into one and marks sources as merged", async () => {
      mockConnection.executeMany.mock.mockImplementation(
        async (sql: string) => {
          if (sql.includes("UPDATE codeatlas_genome SET status"))
            return { rowsAffected: 1 };
          if (sql.includes("INSERT INTO gene_relationships"))
            return { rowsAffected: 1 };
        },
      );
      mockConnection.execute.mock.mockImplementation(async (sql: string) => {
        ...
        return {
          rows: [
            mockGeneRow("gene-1", "Source A"),
            mockGeneRow("gene-2", "Source B"),
          ],
        };
      });
  ...
  describe("splitGene()", () => {
    test("splits gene into children and retires source", async () => {
      ...
      mockConnection.execute.mock.mockImplementation(async (sql: string) => {
        ...
        if (callNum === 1)
          return { rows: [mockGeneRow("gene-src", "Source Gene")] };
      ...
      const childIds = await GenomeService.splitGene(
        "gene-src",
        ["Part A", "Part B"],
        "test",
      );
  ...
  describe("mutateGene()", () => {
    test("successful mutation boosts confidence", async () => {
      ...
      const updateCalls = mockConnection.execute.mock.calls.filter(
        (c: any) =>
          typeof c.arguments[0] === "string" && c.arguments[0].includes("SET"),
      );
      assert.ok(updateCalls.length > 0);
    });
    ...
    test("failed mutation lowers confidence", async () => {
      mockConnection.execute.mock.mockImplementation(async () => ({
        rows: [
          ["gene-abc", "Auth", "Problem", "Solution", 0.8, 3, 0.7, 5, "active"],
        ],
        rowsAffected: 1,
      }));
    ...
    test("throws when gene not found", async () => {
      mockConnection.execute.mock.mockImplementationOnce(async () => ({
        rows: [],
        rowsAffected: 0,
      }));
      await assert.rejects(
  ...
  describe("Immune System — CRISPR-Cas9 Inspired", () => {
    ...
    test("createImmuneGene starts with confidence 0.70 (higher base than normal genes)", async () => {
      ...
      const insertCalls = mockConnection.execute.mock.calls.filter(
        (c: any) =>
          typeof c.arguments[0] === "string" &&
          c.arguments[0].includes("INSERT INTO codeatlas_genome"),
      );
      assert.ok(insertCalls.length > 0);
      const bindVals = JSON.stringify(insertCalls[0].arguments[1]);
      assert.ok(
        bindVals.includes("0.7"),
        "Immune gene should have 0.70 confidence",
      );
    });

    test("buildImmuneContext returns empty for no matches", async () => {
      mockGenerateEmbedding.mock.mockImplementationOnce(async () =>
    ...
    test("CRISPR: immune gene confidence threshold (only > 0.2 confidence returned)", async () => {
      ...
      mockConnection.execute.mock.mockImplementation(
        async (sql: string, binds: any) => {
          ...
          return {
            rows: [
              mockGeneRow("gene-imm-3", "[IMMUNE] Low Confidence", "immune"),
            ],
            rowsAffected: 0,
          };
        },
      ...
      assert.ok(
        queryExecuted,
        "Query must be executed to fetch the full rows after vector search",
      );
    ...
    test("CRISPR: multiple immune genes ranked by vector distance", async () => {
      ...
      const genes = await GenomeService.scanImmuneGenes(
    ...
    test("CRISPR: immune gene retains proper source tracking", async () => {
      ...
      const geneId = await GenomeService.createImmuneGene(
        "Memory Leak",
        "Map allocation in 60fps loop",
        "Use linkIndices pattern",
        "codeatlas-ai",
      );
      ...
      if (insertCall) {
        ...
        assert.equal(
          binds.srcType,
          "feedback",
          'Immune gene source should be "feedback"',
        );
      }
    });
    ...
    test("CRISPR: multiple scans do NOT increment usage_count (scan is passive)", async () => {
      mockConnection.execute.mock.mockImplementation(async (sql: string) => {
        return {
          rows: [mockGeneRow("gene-imm-1", "[IMMUNE] Bug", "immune")],
          rowsAffected: 1,
        };
      });
      ...
      const updateCalls = mockConnection.execute.mock.calls.filter(
        (c: any) =>
          typeof c.arguments[0] === "string" &&
          c.arguments[0].includes("SET usage_count"),
      );
      assert.equal(
        updateCalls.length,
        0,
        "Passive scan should not SET usage_count",
      );
    });
  });
  ...
  describe("Edge Cases", () => {
    ...
    test("upsertGene handles empty embedding", async () => {
      mockGenerateEmbedding.mock.mockImplementationOnce(
