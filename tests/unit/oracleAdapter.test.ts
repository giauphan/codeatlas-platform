...
// Set required env vars before importing OracleAdapter (getDbConfig throws otherwise)
process.env.ORACLE_USER = "ADMIN";
process.env.ORACLE_PASSWORD = "test-password";
process.env.ORACLE_CONN_STRING = "localhost:1521/ORCL";
...
const { OracleAdapter } = await import(
  path.join(srcDir, "database/adapters/oracleAdapter.js")
);
...
describe("OracleAdapter", () => {
  ...
  test("graph operations (detectCircularDependencies, detectGodObjects, detectDeadCode)", async () => {
    ...
    const circular = await adapter.detectCircularDependencies(
      "proj",
      "test-user",
    );
