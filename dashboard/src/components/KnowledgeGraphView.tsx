...
export const KnowledgeGraphView: React.FC<KnowledgeGraphViewProps> = ({
  ...
  useEffect(() => {
    ...
    const moduleCount = prioritizedNodes.filter(
      (n: any) => n.type === "module",
    ).length;
    const nonModuleCount = prioritizedNodes.filter(
      (n: any) => n.type !== "module",
    ).length;
      ...
      setSimNodes((prevNodes) => {
        ...
        for (let i = 0; i < nodes.length; i++) {
          for (let j = i + 1; j < nodes.length; j++) {
            ...
            const minDist =
              n1.type === "module" || n2.type === "module" ? 120 : 75;

            // ⚡ Bolt Optimization: Use squared distance checks before applying expensive Math.sqrt in O(n²) loop
            // Expected Impact: Reduces calculation overhead from 1.35m Math.sqrt/sec down to only evaluating nodes in proximity
        ...
        if (analysis && analysis.graph && analysis.graph.links) {
          analysis.graph.links.forEach((l: any) => {
            ...
            if (sourceNode && targetNode) {
              ...
              const desiredDist =
                sourceNode.type === "module" || targetNode.type === "module"
                  ? 95
                  : 65;
        ...
        nodes.forEach((n) => {
          ...
          // ⚡ Bolt Optimization: Use squared distance checks before applying expensive Math.sqrt in O(n) loop
          // Expected Impact: Reduces calculation overhead from Math.sqrt/sec
          const distSq = dx * dx + dy * dy;
    ...
    >
      ...
      >
        <div>
          <h1
            className="tech-font"
            style={{ fontSize: "2.5rem", margin: 0, fontWeight: 800 }}
          >
            Knowledge Network
          </h1>
          <p style={{ color: "var(--text-muted)" }}>
            Interactive physics-based neural mapping of{" "}
            <span style={{ color: "var(--primary-neon)" }}>
              {analysis?.stats?.totalModules || 0}
            </span>{" "}
            logic clusters.
          </p>
        </div>
      ...
      >
        ...
        >
          ...
          >
            <div style={{ position: "relative", width: "260px" }}>
              ...
              <input
                ...
                placeholder="Search logic clusters..."
                style={{
                  paddingLeft: "3.25rem",
            ...
            {projects && projects.length > 0 && (
              <div style={{ position: "relative", width: "220px" }}>
                <select
                  ...
                  onChange={(e) =>
                    onProjectChange && onProjectChange(e.target.value)
                  }
                  className="glass-input"
                  style={{
                    appearance: "none",
          ...
          >
            ...
            >
              🖱️ <span>Click & Drag background to Pan</span>
            </div>
            <div
              style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}
            >
            ...
            <div
              style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}
            >
              ⚡ <span>Drag individual nodes to inspect</span>
            </div>
          </div>
                    ...
                    {(isHovered || isModule) && (
                      <text
                        ...
                        style={{
                          ...
                          fontFamily: "monospace",
                          textShadow:
                            "0 1px 4px rgba(0,0,0,0.9), 0 0 10px rgba(0,0,0,0.9)",
                        }}
                      >
                        {node.label}
                      </text>
          ...
          {selectedNode ? (
            ...
            >
              ...
              >
                <div
                  style={{
                    ...
                    justifyContent: "center",
                    color:
                      selectedNode.type === "module" ? "#00F0FF" : "#FF00A8",
                  }}
                >
                  {selectedNode.type === "module" ? (
                    <Box size={18} />
              ...
              >
                ...
                >
                  ...
                  >
                    {
                      simulatedLinks.filter(
                        (l) => l.source.id === selectedNode.id,
                      ).length
                    }
                  </div>
                  <div
              ...
              <div
                style={{
                  ...
                  borderRadius: "16px",
                  border: "1px solid rgba(255, 0, 168, 0.1)",
                  fontSize: "0.8rem",
                  lineHeight: 1.4,
                  color: "rgba(255, 255, 255, 0.8)",
                }}
              >
                💡 <b>Spring Physics Tip:</b> Grab this node and shake it to
                disperse clustered logic blocks and visualize relative node
                weights!
              </div>
            </div>
          ) : (
            ...
            <>
              ...
              {[
                {
                  label: "Modules",
                  count: analysis?.stats?.totalModules || 0,
                  icon: Box,
                  color: "#00F0FF",
                },
                {
                  label: "Functions",
                  count: analysis?.stats?.totalFunctions || 0,
                  icon: Code2,
                  color: "#FF00A8",
                },
                {
                  label: "Classes",
                  count: analysis?.stats?.totalClasses || 0,
                  icon: Layers,
                  color: "#FFB400",
                },
                {
                  label: "Variables",
                  count: analysis?.stats?.totalVariables || 0,
                  icon: Activity,
                  color: "#00FF94",
                },
              ].map((item, i) => (
              ...
              {(() => {
                const totalFiles =
                  (analysis?.totalFilesAnalyzed || 0) +
                  (analysis?.totalFilesSkipped || 0);
                const coveragePercent =
                  totalFiles > 0
                    ? Math.round(
                        ((analysis?.totalFilesAnalyzed || 0) / totalFiles) *
                          100,
                      )
                    : 100;
                return (
              ...
              {selectedProjectDir && onDeleteProject && (
                ...
                >
                  ...
                  >
                    Permanently delete the project index, Firestore telemetry,
                    and Oracle AI Database records.
                  </p>
                  <button
                    ...
                    onClick={() => {
                      if (
                        window.confirm(
                          "Are you absolutely sure you want to delete this project's index, telemetry, and Oracle DB data? This cannot be undone.",
                        )
                      ) {
                        onDeleteProject();
                      }
